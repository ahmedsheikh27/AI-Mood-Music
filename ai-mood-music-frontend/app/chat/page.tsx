...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (urlToken) {
      ...
      toast({
        ...
        description: "Please connect to Spotify to use the chat.",
        variant: "destructive",
      })
      router.replace("/")
    }
  ...
  useEffect(() => {
    if (!spotifyToken || messages.length > 0) return

    const startChat = async () => {
      setIsLoading(true)
      try {
        setTimeout(async () => {
          console.log("Calling /chat/start with token:", spotifyToken)
          ...
          setIsLoading(false)
        }, 500) // Delay fetch for smoother runtime
      } catch (error) {
    ...
    startChat()
  }, [spotifyToken, messages.length, toast, router])
  ...
  if (!spotifyToken) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-purple-600 to-indigo-800 text-white">
